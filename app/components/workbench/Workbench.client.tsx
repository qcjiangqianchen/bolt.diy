...
import { visualEditorHtmlAtom, visualEditorCssAtom } from '~/lib/stores/visualEditorStore';
...
/*
 * Slider options and workbench variants (used when Slider component is enabled)
...
export const Workbench = memo(
  ({
    ...
    return (
      chatStarted && (
        <div
          className={classNames('h-full w-full flex flex-col transition-opacity duration-200', {
            'opacity-100': showWorkbench,
            'opacity-0 pointer-events-none': !showWorkbench,
          })}
          style={{
            display: showWorkbench ? 'flex' : 'none',
          }}
        >
          <div className="h-full w-full flex flex-col p-2 lg:p-4">
            <div className="h-full flex flex-col bg-bolt-elements-background-depth-2 border border-bolt-elements-borderColor shadow-sm rounded-lg overflow-hidden">
              <div className="flex items-center px-3 py-2 border-b border-bolt-elements-borderColor gap-1.5">
                ...
                />
                {/* View label — always says Preview regardless of mode */}
                ...
                {isDeployed && selectedView !== 'code' && selectedView !== 'diff' && (
                  <button
                ...
                {isDeployed && currentMeta?.deployedUrl && selectedView !== 'code' && selectedView !== 'diff' && (
                  <a
                ...
                <div className="flex overflow-y-auto items-center gap-1">
                  ...
                  {/* Deploy button — always visible in the workbench header top-right */}
                  <DeployButton />
                </div>
              ...
              <div className="relative flex-1 overflow-hidden">
                ...
                <View initial={{ x: '100%' }} animate={{ x: selectedView === 'visual' ? '0%' : '100%' }}>
                  <VisualEditor />
                </View>
              </div>
            </div>
...
const UseInChatButton = memo(() => {
  ...
  const handleUseInChat = () => {
    ...
    const prompt = [
      'I designed the following page in the visual builder. Please use this as a starting point and convert it into a full, polished webpage:',
      '',
      '```html',
      '<!DOCTYPE html>',
      '<html>',
    ...
    navigator.clipboard
      .writeText(prompt)
      .then(() => {
        toast.success('Copied to clipboard! Paste it into the chat.', {
          icon: <div className="i-ph:clipboard-text text-accent-500" />,
        });
      })
      .catch(() => {
        // Fallback: write to a textarea and select
        toast.info('Could not copy automatically — please copy manually from the console.');
  ...
  return (
    <button
      ...
      className="flex items-center gap-1 px-2 py-1 text-xs rounded-md transition-colors ml-1 bg-purple-500/20 text-purple-500 border border-purple-500/30 hover:bg-purple-500/30"
      title="Copy exported HTML to clipboard for use in the AI chat"
    >
...
UseInChatButton.displayName = 'UseInChatButton';
